...
app.put("/jokes/:id", (req, res) => {
    ...
    if (jokeIndex !== -1) {
        ...
        if (!updatedJokeText || !updatedJokeType) {
            return res.status(400).json({ error: "Missing required fields" });
        }
        jokes[jokeIndex].jokeText = updatedJokeText;
        jokes[jokeIndex].jokeType = updatedJokeType;
...
app.delete("/all", (req, res) => {
    // Check for the master key in the request headers
    console.log("Request Headers:", req.query); // Log the request headers for debugging
    const providedKey = req.query.key; // Get the master key from the query parameters
    if (providedKey !== masterKey) {
        return res.status(403).json({ error: "Forbidden: Invalid master key" });
    }
    jokes = []; // Clear the jokes array
